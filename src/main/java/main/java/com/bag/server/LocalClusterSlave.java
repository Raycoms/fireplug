...
public class LocalClusterSlave extends AbstractRecoverable
{
    ...
    public LocalClusterSlave(final int id, @NotNull final ServerWrapper wrapper, final int localClusterId, final ServerInstrumentation instrumentation)
    {
        ...
        timer.scheduleAtFixedRate(new CrashDetectionSensor(positionToCheck, new ServiceProxy(3000 + id, String.format(LOCAL_CONFIG_LOCATION, localClusterId)), String.format(LOCAL_CONFIG_LOCATION, localClusterId), id, pool.borrow()), 10000, 8000);
