...
public class GlobalClusterSlave extends AbstractRecoverable
{
    ...
    GlobalClusterSlave(final int id, @NotNull final ServerWrapper wrapper, final ServerInstrumentation instrumentation)
    {
        ...
        timer.scheduleAtFixedRate(new CrashDetectionSensor(positionToCheck, new ServiceProxy(1000 + this.idClient, GLOBAL_CONFIG_LOCATION), GLOBAL_CONFIG_LOCATION, id, pool.borrow(), -1), 10000, 10000);
    }
