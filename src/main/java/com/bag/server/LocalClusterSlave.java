...
import com.bag.util.storage.LocalSlaveUpdateStorage;
import com.bag.util.storage.NodeStorage;
import com.bag.util.storage.RelationshipStorage;
import com.bag.util.storage.SignatureStorage;
...
public class LocalClusterSlave extends AbstractRecoverable
{
    ...
    public LocalClusterSlave(final int id, @NotNull final ServerWrapper wrapper, final int localClusterId, final ServerInstrumentation instrumentation, final boolean isPrimary)
    {
        ...
        timer.scheduleAtFixedRate(new LoadSensor(pool.borrow(), loadProxy, id, wrapper.getDataBaseAccess().getName(), isPrimary), 15000, 15000);
    }
    ...
    private void handlePerformanceUpdateMessage(final Input input, final Kryo kryo)
    {
        ...
        if (performanceMap.size() == proxy.getViewManager().getCurrentViewN() && wrapper.getGlobalCluster() != null)
        {
            ...
            for(final LoadSensor.LoadDesc load : performanceMap.values())
            {
                ...
                {
                    slaveCount++;
                    Log.getLogger().info("Detected CPU usage: " + load.getCpuUsage());
                    if (instance == load.getInstance() && (load.getCpuUsage() > BORDER_CPU_USAGE && load.getAllocatedMemory() > 1800000000) ||
